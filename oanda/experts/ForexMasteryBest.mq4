...
extern int analysis_days = 100;
extern int maxValue = 2000;
...
double lots;
int strategies[28];
int strategiesPeriod[28];
int strategiesMax[28];
...
//----
   ...
   for (x = 0; x < ARRSIZE; x++) {
      ...
      if(handle>0)
      {
         ...
         strategiesPeriod[x] = StrToInteger(resultString[1]);
         strategies[x] = StrToInteger(resultString[2]);
         strategiesMax[x] = StrToInteger(resultString[3]);
         checked2[x] = 1;
         continue;
      }
      for(p = 0; p < 3; p++) {
         ...
         if (s[p][x] > 0) {
            ...
            max = 0;
            for (int j = 1; j <= 30; j++) {
               if (j == 4 || j == 23 || j == 24 || j == 25) continue;
               val = iCustom(symbol, period, "cuSpanTimeClose", j, no, 0, 4, 0);
               if (val > max && val != EMPTY_VALUE) {
                  max = val;
                  sm[p][x] = max;
                  s[p][x] = j;
                  Print("Checking for period " + period + " sp: " + s[p][x]
                     + ", val: " + sm[p][x]
                     + ", checked: " + checked[p][x]);
               }//end if
            }//end for
      ...
      for(p = 0; p < 3; p++) {
         ...
         if (sm[p][x] > max) {
            max = sm[p][x];
            strategies[x] = s[p][x];
            strategiesPeriod[x] = period;
            strategiesMax[x] = sm[p][x]; 
         }
      }
      checked2[x] = 1;
      FileDelete(fnSt);
      FileAppend(fnSt, symbol + "," + strategiesPeriod[x] + "," + strategies[x] + "," + strategiesMax[x]);
   }//end for
         ...
         for (x = 0; x < ARRSIZE; x++) {
            ...
            if (strategies[x] > 0) {
               int st = strategies[x];
               int speriod = strategiesPeriod[x];
               int smax = strategiesMax[x];
               if (smax < maxValue) continue;
               message = magic + ", " + speriod + ", " + st + ", " + 1;
               check = get_strategy_result(st, symbol, speriod, 1, 0);
               infobox = infobox + "\nSymbol: " + symbol + ", Best: " + st + "(" + get_strategy_name(st) 
                           + "), Change: " + check
                           + ", Period: " + TimeframeToString(speriod)
                           + ", Max: " + smax
                           + ", Message: " + message
                           + ", Spread: " + DoubleToStr(MarketInfo(symbol, MODE_SPREAD), 0)
                           ;
               gtotal = gtotal + strategiesMax[x];
               if (check == 1) {
