...
int start()
  {
//----
      string filename = Symbol()+"/heiken2_" + Year() + "_" + Month() + "_" + Day()  + ".txt";
      trailing_stop();
      double val2, val3;
      infobox="";
...
int createorder(string symbol, int timeperiod, int type, string message)
{
   ...
   if (type == 1) {
      sl = Ask - (200 * Point);
      tp = 0; //Ask + (500 * Point);
      ticket=OrderSend(symbol,OP_BUY,Lots,asks,3,sl,tp,message+", 1.2",MAGICMA,0,Green);
      ...
      sl = Bid + (200 * Point);
      tp = 0; //Bid - (500 * Point);
       ticket=OrderSend(symbol,OP_SELL,Lots,bids,3,sl,tp,message+", 1.2",MAGICMA,0,Red);
