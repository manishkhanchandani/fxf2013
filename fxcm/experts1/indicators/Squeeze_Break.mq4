   ...
   for(int i = limit; i >= 0; i--) //main indicator FOR loop
      ...
      if (Squeeze[i] == 1 && Squeeze[i+1] == 0 && Momentum[i] > 0 && i == 0) // a cross above zero line and Mom > 0
         ...
         if (Last_Alert_Time != Time[0]) 
            {
            if (Alert_On == true) SendAlert("Buy"); //Alert("Alert: Possible Breakout - "+Symbol()+" - "+TimeToStr(TimeLocal()));
      ...
      if (Squeeze[i] == 1 && Squeeze[i+1] == 0 && Momentum[i] < 0 && i == 0 )   // a cross above zero line and Mom < 0
         ...
         if (Last_Alert_Time != Time[0]) 
            {
            if (Alert_On == true) SendAlert("Sell"); //Alert("Alert: Possible Breakout - "+Symbol()+" - "+TimeToStr(TimeLocal())); 
            Last_Alert_Time = Time[0];          
...
void SendAlert(string dir)
{
   ...
   Alert("Alert: Possible ", dir, " Breakout - "+Symbol()+" - "+TimeToStr(TimeLocal())); 
   //SendMail(Symbol() + " @ " + per + " - " + " Possible "+ dir + " Breakout ", " Possible "+dir+" Breakout on " + Symbol() + " @ " + per + " as of " + TimeToStr(TimeLocal()));
}