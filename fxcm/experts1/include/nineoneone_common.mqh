...
void custom_start()
{
   ...
   for (int k=0; k<3; k++) {
      ...
      for (int j=0; j<9; j++) {
         ...
         gold(symbol, period_array[j], number);
...
int gold(string symbol, int timeframe, int shift)
{
   ...
   if (result1 == 1 && result2 == 1 && result3 == 1) {
      if (range2 < -25 || iLow(symbol, timeframe, number2) < buy2) {
         result = 1;
      }
   } else if (result1 == -1 && result2 == -1 && result3 == -1) {
      if (range2 > -75 || iHigh(symbol, timeframe, number2) > sell2) {
         result = -1;
      }
   }
   return (result);
