...
extern double custom_tp = 0;
extern double custom_sl = 0;
...
int trailing_stop(int its, int ts)
{
      ...
      if(OrderType()<=OP_SELL && OrderMagicNumber()==MAGICMA)  // check for symbol
         ...
         orderbox = StringConcatenate(orderbox, " - ordertype: ", OrderType());
            if(OrderType()==OP_BUY) {
               ...
               {                 
                  orderbox = StringConcatenate(orderbox, " - pt*its: ", (pt*its));
                  orderbox = StringConcatenate(orderbox, " - bids-OrderOpenPrice(): ", (bids-OrderOpenPrice()));
                  if(bids-OrderOpenPrice()>pt*its)
                  {
                     orderbox = StringConcatenate(orderbox, " - OrderStopLoss(): ", OrderStopLoss());
                     orderbox = StringConcatenate(orderbox, " - bids-pt*ts: ", (bids-pt*ts));
                     if(OrderStopLoss()<bids-pt*ts)
                     {
                        Alert(OrderSymbol(), ", Modify Buy");
                        OrderModify(OrderTicket(),OrderOpenPrice(),bids-pt*ts,OrderTakeProfit(),0,Green);
                        return(0);
                     }
                  }
               }//end if
               ...
               && OrderProfit() > 0
                 ...
                 {             
                  ...
                  if((OrderOpenPrice()-asks)>(pt*its))
                    {
                     orderbox = StringConcatenate(orderbox, " - OrderStopLoss(): ", OrderStopLoss());
                     orderbox = StringConcatenate(orderbox, " - asks+pt*ts: ", (asks+pt*ts));
                     if((OrderStopLoss()>(asks+pt*ts)) || (OrderStopLoss()==0))
                       {
                        Alert(OrderSymbol(), ", Modify Sell");
                        OrderModify(OrderTicket(),OrderOpenPrice(),asks+pt*ts,OrderTakeProfit(),0,Red);
                        return(0);
                       }
                    }
                 }
