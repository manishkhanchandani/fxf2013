...
#define EANAME "#2011-06_Ichimoku EA v1.1"
...
extern int    magic = 2011644;
extern string ordercomment = "Ichimoku EA v1.1";
...
extern int atrTPmultiplier = 5;
extern int atrSLmultiplier = 4;
extern int atrCalcTF = 5;
...
int start()
  {
    ...
    if (stopTradingOnFriday)
     {
      ...
      if ((gmtTime(GMTShift,1) == 5 && gmtTime(GMTShift,0) > (fridayCloseTime - 1))
           || (gmtTime(GMTShift,1) == 0 && gmtTime(GMTShift,0) < 23)
    ...
    commentText = commentText + StringConcatenate("  TF:",timeFrame," | open orders: ",getOrdersPerMagic(magic)," | max. open orders: ",maxOpenOrders," | pairs to trade: ",ArraySize(pairs),"\n");

    for(int c = 0; c < ArraySize(pairs); c++)
      {
        // if(TimeDayOfWeek(TimeLocal()) == 5 && TimeHour(TimeLocal()) >= 12) break;
...
if (tenkanTrend == "SHORT"
  ...
  commentText = commentText + StringConcatenate("\n",symbol," :  tradeSignal: ",ichiTrend," (tenk: ",tenkanTrend, " | kij: ",kijunTrend," | senk: ",senkouTrend," | chik: ",chikouTrend,"");
