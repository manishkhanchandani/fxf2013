...
int strategy(string symbol, int number)
{
   ...
   int limit;
   int period[3] = {PERIOD_H4, PERIOD_H1, PERIOD_M30};
   limit = 3;  
   string mes;
   int i;
   //ichimoku
   ...
   for (i=0; i < limit; i++) {
      ...
      if (condition_buy)
         condition[number][i][0] = 1;
      else if (condition_sell)
         condition[number][i][0] = -1;
      if (condition_buy2)
         condition[number][i][1] = 1;
      else if (condition_sell2)
         condition[number][i][1] = -1;
      if (condition[number][i][0] == 1 && condition[number][i][1] == 1)
         call_order_creation(true, false, condition_name, symbol, period[i], number, i, bids);
      else if (condition[number][i][0] == -1 && condition[number][i][1] == -1)
         call_order_creation(false, true, condition_name, symbol, period[i], number, i, bids);
      infobox = StringConcatenate(infobox, " - (", condition[number][i][0], ",", condition[number][i][1], ")");
   } 
...
int call_order_creation(bool condition_buy, bool condition_sell, string condition_name, string symbol, int timeperiod, int number, int i, double bids)
{
   string mes = "";
      if (condition_buy) {
         ...
         createorder(symbol, timeperiod, 1, condition_name+" "+build + ", P("+ TimeframeToString(timeperiod)+" - : "+ condition[number][i][0]+ ","+ condition[number][i][1]+ ")");
      } else if (condition_sell) {
         mes = "Sell";
         if (amount_type[number][i] != -1) {
            amount[number][i] = bids;
   ...
   condition[number][i][0] = 0;
   condition[number][i][1] = 0;
      return (0);
}
