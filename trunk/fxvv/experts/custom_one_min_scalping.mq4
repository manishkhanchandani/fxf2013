...
extern int number = 1;
extern int maxorders = 35;
extern double lots = 0.50;
extern int custom_tp_fpips = 20;
...
{   
   ...
   int result = 0;
   if (High[i] < ima25 && High[i] < ima50 && High[i] < ima100 && signal == -1 && previous1 == 0 && Open[i] > Close[i] && counter < 0) {
      Print("Result: Bearish");
      previous2 = 0;
      previous1 = 1;
      message = StringConcatenate("Down with trend ", trend, ", Account Number: ", AccountNumber(), ", is Account Type: ", demo, ", Company: ", AccountCompany());
      SendAlert(message);
      result = -1;
      counter = 0;
   } else if (Low[i] > ima25 && Low[i] > ima50 && Low[i] > ima100 && signal == 1 && previous2 == 0 && Open[i] < Close[i] && counter > 0) {
      Print("Result: Bullish");
      previous1 = 0;
      previous2 = 1;
      message = StringConcatenate("Up with trend ", trend, ", Account Number: ", AccountNumber(), ", is Account Type: ", demo, ", Company: ", AccountCompany());
      SendAlert(message);
      result = 1;
      counter = 0;
   } else if (ima25 < ima50 && ima25 > ima100) {
