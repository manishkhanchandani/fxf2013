...
extern int InitialTrailingStop = 0;
...
int create_reverse_order(string symbol)
  {
   int cnt=0;
   int i;
//----
...
int strength()
{
   ...
   for (x = 0; x < PAIRSIZE; x++) {
      ...
      infobox = infobox + "\nNumber: " + x +
         ...
         ", Spread: " + MarketInfo(condition_symbol[x], MODE_SPREAD);
      if (condition_type[x] != 0) {
         val = iCCI(condition_symbol[x],PERIOD_M15,45,PRICE_CLOSE,0);
         val2 = iCCI(condition_symbol[x],PERIOD_M15,45,PRICE_CLOSE,1);
