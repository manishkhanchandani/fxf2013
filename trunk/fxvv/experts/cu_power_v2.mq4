...
extern string build = "2.2";
extern int InitialTrailingStop = 150;
extern int TrailingStop = 150;
extern bool markethours = false;

extern bool stop_order_on_loss = false;
extern double max_loss_per_day = -200;
...
extern bool only_particular_symbol = true;
extern string only_symbol = "EURUSD"; //e.g. USDJPY
...
int strategy()
{
   ...
   if (openTime != Time[0]) {
      string mySymbol;
      infobox = "";
      filename = "power/" + Year() + "_" + Month() + "_" + Day()  + ".txt";
      //FileDelete(filename);
      impbox = "\n";
      createbox = "\n";
      Sleep(1000);
      ...
      infobox = StringConcatenate(infobox, "\nLots: ", Lots,
         ...
         ", maxorders: ", maxorders,
         ", hour: ", hour,
         ", lot: ",lot, "top: ", top, ", Bottom: ", bottom, ", Maxspread: ", maxspread, "\n");
      int index;
      ...
      for (index = 0; index < limit; index++) {
         ...
         strength_individual = strength_individual + "\n" + mySymbol + ": ahigh: " + aHigh[index] + ", alow: " + aLow[index] + ", aBid: " + aBid[index]
             + ", aAsk: " + aAsk[index] + ", aRange: " + aRange[index] + ", aRatio: " + aRatio[index] + ", aLookup: " + aLookup[index]
             + ", aStrength: " + aStrength[index]; 
         if (index % 2 == 0) {
            infobox = infobox + "\n";
         }
         infobox = infobox + ":"+ mySymbol + ", aLookup: " + aLookup[index]
             + ", aStrength: " + aStrength[index] + ", "; 
         check = lookupcurrency2(mySymbol);
         infobox = infobox + ":"+ mySymbol + ", check: " + check + ", ";
         if (check == 1 && aLookup[index] > aStrength[index]) {
            reverse_bid_positive(1, mySymbol);
            infobox = infobox + ", Create Buy Order";
            SendAlert("Bullish", mySymbol, PERIOD_H1);
            createorder(mySymbol, timeperiod, 1, TimeframeToString(timeperiod)); 
         } else if (check == -1 && aLookup[index] < aStrength[index]) {
            reverse_bid_positive(-1, mySymbol);
   ...
   FileDelete("strength.txt");
   FileAppend("strength.txt", strength_individual + strength);
      openTime = Time[0];
...
int createorder(string symbol, int timeperiod, int type, string message)
{
   ...
   if (markethours) {
      if (hour == 16 || hour == 17 || hour == 18 || hour == 19 || hour == 20 || hour == 21 || hour == 22 || hour == 23) {
         createbox = createbox + " Hour: " + hour + " so NO TRADING";
         return (0);
      }
      
      if (hour == 0 && Minute() < 30) {
         createbox = createbox + " Hour: " + hour + " and Minute is " + Minute() + " so NO TRADING";
         return (0);
      }
...
int lookupcurrency2(string symbol)
{
   timeperiod = Period();
         double val, val2, val3, val4, val5, val6, cci, cci2;
         double h1,h2,h3,h4;
         ...
         //infobox = infobox + "\nChecking Heiken";
         //infobox = infobox + "H1: " + h1 + ", H2: " + h2 + ", H3: " + h3 + ", H4: " + h4;
         cci = iCCI(symbol, Period(),45,PRICE_CLOSE,1);
         cci2 = iCCI(symbol, Period(),45,PRICE_CLOSE,2);
         //infobox = infobox + " - Checking CCI";
         //infobox = infobox + "cci: " + cci + ", cci2: " + cci2;
         ...
         //infobox = infobox + " - Checking MACD";
         //infobox = infobox + "val2: " + val2 + ", val3: " + val3 + ", val4: " + val4 + ", val5: " + val5;
         //infobox = infobox + " - Checking MACD";
         infobox = infobox + (h1 < h2 && cci > 100 && val2 > val3) + "," + (h3 > h4 || cci2 < 100 || val4 < val5) 
         + "," + (h1 > h2 && cci < -100 && val2 < val3) + "," + (h3 < h4 || cci2 > -100 || val4 > val5);
         if ((h1 < h2 && cci > 100 && val2 > val3) && (h3 > h4 || cci2 < 100 || val4 < val5)) {
            return (1);
         } else if ((h1 > h2 && cci < -100 && val2 < val3) && (h3 < h4 || cci2 > -100 || val4 > val5)) {
            return (-1);
         }
         return (0);
}
...
int data(string symbol, string type)
{
   ...
   for (i = 0; i < limit; i++) {
      ...
      if (val2 > val3) {
         ...
         databox = databox + " - SELL";
      } else {
         databox = databox + " - CONSOLIDATE";
      }
   }
   for (i = 0; i < limit; i++) {
   ...
   for (i = 0; i < limit; i++) {  
      ...
      databox = databox + "\nChikouspan (" + TimeframeToString(period[i]) + "): chinkouspan: " + DoubleToStr(chinkouspan, digit);
   ...
   for (i = 0; i < limit; i++) {  
      val = iADX(symbol,period[i],14,PRICE_CLOSE,MODE_MAIN,0);
      val2 = iADX(symbol,period[i],14,PRICE_CLOSE,MODE_PLUSDI,0);
      val3 = iADX(symbol,period[i],14,PRICE_CLOSE,MODE_MINUSDI,0);
      databox = databox + "\nADX (" + TimeframeToString(period[i]) + "): val: " + DoubleToStr(val, digit) + ", +DI: " + DoubleToStr(val2, digit) + ", -DI: " + DoubleToStr(val3, digit);
      if (val > 20 && val2 > val3) {
         databox = databox + " - BUY";
