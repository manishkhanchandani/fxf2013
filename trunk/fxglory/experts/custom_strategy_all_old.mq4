...
int strategy(string symbol, int number)
{
   ...
   for (i=0; i < limit; i++) {
      ...
      condition_buy = (iClose(symbol, period[i], 1) > spanHigh && iOpen(symbol, period[i], 1) < spanHigh && dip > dim);
      condition_sell = (iClose(symbol, period[i], 1) < spanLow && iOpen(symbol, period[i], 1) > spanLow && dip < dim);
