            ...
            if(OrderType()==OP_BUY) {
               ...
               && OrderProfit() > 0
                 ...
                 {             
                  ...
                  if((OrderOpenPrice()-bids)>(pt*its))
                    {
                     orderbox = StringConcatenate(orderbox, " - OrderStopLoss(): ", OrderStopLoss());
                     orderbox = StringConcatenate(orderbox, " - bids+pt*ts: ", (bids+pt*ts));
                     if((OrderStopLoss()>(bids+pt*ts)) || (OrderStopLoss()==0))
                       {
                        Alert(OrderSymbol(), ", Modify Sell");
                        OrderModify(OrderTicket(),OrderOpenPrice(),bids+pt*ts,OrderTakeProfit(),0,Red);
                        return(0);
                       }
                    }
                 }
