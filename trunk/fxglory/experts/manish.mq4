...
void custom_start()
{
   double a = (10000 * 2) / 100; // 2 is riskfactor
   double b = a / 5;
   double c = b / 100;
   double d = NormalizeDouble(c, 2);
   infobox = StringConcatenate(
      "Account Free Margin: ", AccountFreeMargin(),
      ", Risk: 2%, Lots Available: ", d,
      ", Total Currency: 12, Lot per currency: ", (d / 12),
      "\n",
      infobox
   );
   Comment(infobox);
      ...
      if(OrderMagicNumber()!=MAGICMA || OrderSymbol()!=symbol) continue;
         ...
         if (OrderProfit() > 0) {
            if (OrderType()==OP_BUY) {
               ...
               diff = MarketInfo(OrderSymbol(), MODE_BID) - OrderOpenPrice();
               diff2 = diff / MarketInfo(OrderSymbol(), MODE_POINT);
               amt = MarketInfo(OrderSymbol(), MODE_BID)-(MarketInfo(OrderSymbol(), MODE_POINT)*100);
               ...
               diff = OrderOpenPrice() - MarketInfo(OrderSymbol(), MODE_ASK);
               diff2 = diff / MarketInfo(OrderSymbol(), MODE_POINT);
               amt = MarketInfo(OrderSymbol(), MODE_ASK)+(MarketInfo(OrderSymbol(), MODE_POINT)*100);
               if (sl_price[cur_num] == 0) {
