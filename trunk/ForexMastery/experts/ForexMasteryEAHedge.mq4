...
//----
   ...
   if (opentime != iTime(Symbol(), PERIOD_M5, 0)) {
      ...
      int    element_count=ArrayResize(AryMax, (24 * 5));
      int high1 = -999999;
      int high2 = -999999;
      int high3 = -999999;
      int high4 = -999999;
      ...
      max = -999999;
      for (x = 0; x < element_count; x++) {
         if (x == high3 || x == high2 || x == high1) continue;
         if (AryMax[x] > max) {
            high4 = x;
            max = AryMax[x];
         }
      ...
      int check;
      x = high1;
      final_strategy = AryStrategy[x];
      final_max = AryMax[x];
      final_period = periods[AryPeriod[x]];
      message = magic + ", " + final_period + ", " + final_strategy;
      infobox = infobox + "\n\nNumber: " + x + ", value = " + AryMax[x]
          + ", Strategy = " + AryStrategy[x]
          + ", Strategy Name = " + get_strategy_name(AryStrategy[x])
           + ", Period = " + TimeframeToString(periods[AryPeriod[x]]) + ", Message: " + message;
      
      check = get_strategy_result(final_strategy, symbol, final_period, 1, 0);
      ...
      x = high2;final_strategy = AryStrategy[x];
      final_max = AryMax[x];
      final_period = periods[AryPeriod[x]];
      message = magic1 + ", " + final_period + ", " + final_strategy;
      infobox = infobox + "\n\nNumber: " + x + ", value = " + AryMax[x]
          + ", Strategy = " + AryStrategy[x]
          + ", Strategy Name = " + get_strategy_name(AryStrategy[x])
           + ", Period = " + TimeframeToString(periods[AryPeriod[x]]) + ", Message: " + message;
      ...
      infobox = infobox + "\nSymbol: " + symbol + ", Best: " + final_strategy + "(" + get_strategy_name(final_strategy) 
                  ...
                  ;
      if (check == 1) {
         create_order(symbol, 1, lots, magic1, message, 0, 0);
      } else if (check == -1) {
         create_order(symbol, -1, lots, magic1, message, 0, 0);
      }
      ...
      final_max = AryMax[x];
      final_period = periods[AryPeriod[x]];
      message = magic2 + ", " + final_period + ", " + final_strategy;
      infobox = infobox + "\n\nNumber: " + x + ", value = " + AryMax[x]
          + ", Strategy = " + AryStrategy[x]
          + ", Strategy Name = " + get_strategy_name(AryStrategy[x])
           + ", Period = " + TimeframeToString(periods[AryPeriod[x]]) + ", Message: " + message;
      ...
      infobox = infobox + "\nSymbol: " + symbol + ", Best: " + final_strategy + "(" + get_strategy_name(final_strategy) 
                  ...
                  ;
      if (check == 1) {
         create_order(symbol, 1, lots, magic2, message, 0, 0);
      } else if (check == -1) {
         create_order(symbol, -1, lots, magic2, message, 0, 0);
      }
      x = high4;
      ...
      final_max = AryMax[x];
      final_period = periods[AryPeriod[x]];
      message = magic3 + ", " + final_period + ", " + final_strategy;
      infobox = infobox + "\n\nNumber: " + x + ", value = " + AryMax[x]
          + ", Strategy = " + AryStrategy[x]
          + ", Strategy Name = " + get_strategy_name(AryStrategy[x])
           + ", Period = " + TimeframeToString(periods[AryPeriod[x]]) + ", Message: " + message;
      ...
      infobox = infobox + "\nSymbol: " + symbol + ", Best: " + final_strategy + "(" + get_strategy_name(final_strategy) 
                  ...
                  ;
      if (check == 1) {
         create_order(symbol, 1, lots, magic3, message, 0, 0);
      } else if (check == -1) {
         create_order(symbol, -1, lots, magic3, message, 0, 0);
      }
      
      string fn = "forexmasteryEAHedge_"+symbol+".txt";
      FileDelete(fn);
