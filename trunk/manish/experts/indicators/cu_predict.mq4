...
{
   filename = Year() + "_" + Month() + "_" + Day()  + "/predict_" + symbol + "_"+Period()+"_.txt";
   infobox = StringConcatenate(infobox, "\n", symbol, " (", number, ")");
   ...
   if (MarketInfo(symbol, MODE_BID) > (open + tenpercent)) {
      ...
      if (sell[number] == 1 && opentime[number] != iTime(symbol, Period(), 0)) {
         ...
         FileAppend(filename, "Buy1: " + TimeToStr(TimeCurrent()));
      }
      ...
      if (buy[number] == 1 && opentime[number] != iTime(symbol, Period(), 0)) {
         result[number] = -1;
         opentime[number] = iTime(symbol, Period(), 0);
         FileAppend(filename, "Sell1: " + TimeToStr(TimeCurrent()));
      }
   } 
   ...
   if (MarketInfo(symbol, MODE_BID) > (open + twentyfive) && opentime2[number] != iTime(symbol, Period(), 0)) {
      ...
      FileAppend(filename, "Buy2: " + TimeToStr(TimeCurrent()));
   } else if (MarketInfo(symbol, MODE_BID) < (open - twentyfive) && opentime2[number] != iTime(symbol, Period(), 0)) {
      result2[number] = -1;
      opentime2[number] = iTime(symbol, Period(), 0);
      FileAppend(filename, "Sell2: " + TimeToStr(TimeCurrent()));
   } 
   ...
   if (result[number] == 1) {
      if (MarketInfo(symbol, MODE_BID) > (open + tenpercent + twentyfive)) {
         infobox = StringConcatenate(infobox, " - Buy Goal Reached");
         FileAppend(filename, "Buy1 Goal Reached: " + TimeToStr(TimeCurrent()));
      }
   } else if (result[number] == -1) {
      if (MarketInfo(symbol, MODE_BID) < (open - (tenpercent + twentyfive))) {
         infobox = StringConcatenate(infobox, " - Sell Goal Reached");
         FileAppend(filename, "Sell1 Goal Reached: " + TimeToStr(TimeCurrent()));
      }
   }
   if (result2[number] == 1) {
      if (MarketInfo(symbol, MODE_BID) > (open + twentyfive + twentyfive)) {
         infobox = StringConcatenate(infobox, " - Buy2 Goal Reached");
         FileAppend(filename, "Buy2 Goal Reached: " + TimeToStr(TimeCurrent()));
      }
   } else if (result2[number] == -1) {
      if (MarketInfo(symbol, MODE_BID) < (open - (twentyfive + twentyfive))) {
         infobox = StringConcatenate(infobox, " - Sell2 Goal Reached");
         FileAppend(filename, "Sell2 Goal Reached: " + TimeToStr(TimeCurrent()));
      }
   }
