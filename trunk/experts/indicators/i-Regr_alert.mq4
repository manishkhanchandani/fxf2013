...
// alert added by http://www.automatedtradingsoftware.com March 3rd, 2009
...
//-----------------------------------Std-----------------------------------------------------------------------------------
  ...
  if(Ask >= sqh[i0] && AlertOn == true)Alerts(1);
  if(Bid <= sql[i0] && AlertOn == true)Alerts(2);
...
void Alerts(int Direction)
{
   ...
   if(Time[0] > AlertLastTime ) {
      AlertLastTime = Time[0];
      PlaySound(SoundFile);
