...
int getallinfo(string symbol)
{
   double high, low, bid, ask, point, spread, digits;
      ...
      int z;
         ...
         aStrength = 9-aLookup;
         if (opentime != Time[0]) {
            string name = "lookup_"+Time[1];
            create_label(name, DoubleToStr(aLookup, 1), Red, Time[1], High[1] + (50 * Point));
            opentime = Time[0];
         }
...
void create_label(string name, string text, color textcolor, datetime time1, double price1)
{
   Print("Creating Label " + text);
   if (ObjectCreate(name, OBJ_LABEL, 0, time1, price1)) {
