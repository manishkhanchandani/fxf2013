...
extern string EA_Version = "1.3"; /*1.1 changes: settings shown*/
...
//----
   ...
   if (opentime != iTime(Symbol(), PERIOD_M5, 0)) {
      ...
      if (opentimeOrders != iTime(Symbol(), PERIOD_M15, 0)) {
         opentimeOrders = iTime(Symbol(), PERIOD_M15, 0);
         getStrategies();
      }
      for (k = 0; k<totalSymbols;k++) {
         ...
         infobox = infobox + "\nSymbol: " + final_symbol[k] + ", Best: " + final_strategy[k] + "(" + get_strategy_name(final_strategy[k]) 
                     ...
                     ;
         if (check == 1) {
            create_order(final_symbol[k], 1, lots, magic, message, 0, 0);
         } else if (check == -1) {
            create_order(final_symbol[k], -1, lots, magic, message, 0, 0);
         }
      }
   ...
   for (k = 0; k<totalSymbols;k++) {
      ...
      zerobox = zerobox + "\nSymbol: " + final_symbol[k] + ", Best: " + final_strategy[k] + "(" + get_strategy_name(final_strategy[k]) 
                     ...
                     ;
      if (check == 1) {
         create_order(final_symbol[k], 1, lots, magic_0, message, 0, 0);
      } else if (check == -1) {
         create_order(final_symbol[k], -1, lots, magic_0, message, 0, 0);
      }
   }
