...
int getmaxPips(string symbol, int period, int lastBar, int currentBar, int type)
{
   ...
   if (type == 1) {
      ...
      pips = (numh - iClose(symbol, period, currentBar)) / MarketInfo(symbol, MODE_POINT);
   } else if (type == -1) {
      num = iLowest(symbol, period, MODE_LOW, count, currentBar);
      numh = iLow(symbol, period, num);
      pips = (iClose(symbol, period, currentBar) - numh) / MarketInfo(symbol, MODE_POINT);
   }
   return (pips);
}
