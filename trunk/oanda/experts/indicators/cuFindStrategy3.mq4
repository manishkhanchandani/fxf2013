...
//----
   if (opentime != Time[0]) {
      ...
      for (strategy = 1; strategy <= 28; strategy++) {
         gtotal = 0;
         for (start=300; start <= 1500; start = start + 300) {
            end = start - 300;
      ...
      string inference = "\nMax Gtotal For strategy: " + best + "(" + get_strategy_name(best) + ") is " + max + "\n\n";
      string filename = symbol + "/cuFindStrategy3_"+period+".txt";
      FileDelete(filename);
