...
int strength()
{
   ...
   for (index = 0; index < ARRSIZE; index++) {
      ...
      if (cur_period) {
         ...
         aHigh1[index] = iHigh(mySymbol, PERIOD_D1, 0);
         aLow1[index]      = iLow(mySymbol, PERIOD_D1, 0); 
         aHigh2[index] = iHigh(mySymbol, PERIOD_D1, 1);
         aLow2[index]      = iLow(mySymbol, PERIOD_D1, 1); 
         aHigh[index] = (aHigh1[index] + aHigh2[index]) / 2;
         aLow[index]      = (aLow1[index] + aLow2[index]) / 2;
         //aHigh[index] = iHigh(mySymbol, PERIOD_D1, 0);
         //aLow[index]      = iLow(mySymbol, PERIOD_D1, 0); 
   ...
   for (x = 0; x < PAIRSIZE; x++) {
      ...
      if (type != 0) {
         if (current_currency) {
            if (condition_symbol[x] != Symbol()) {
               continue;
            }
         }
         high = iHigh(condition_symbol[x], period, 1);
         low = iLow(condition_symbol[x], period, 1);
         totalmove = high - low;
         twentyfivepercent = totalmove * 0.25;
         tenpercent = totalmove * 0.10;
         currentopen = iOpen(condition_symbol[x], period, 0);
...
void calculate(int number)
  {
      ...
      if (aMeter[number] >= top) {
         for (x = 0; x < PAIRSIZE; x++) {
            ...
            if (tmp >= aMeter[x] && aMeter[x] <=  bottom) {
               tmp = aMeter[x];
               tmpCur = x;
